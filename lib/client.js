	...
	factory: (require) => {
		...
		function ServerRow({ server, onChanged, onEdit }) {
			...
			return createElement("div", { className: "mm_row", key: server.id, "data-open": open ? "true" : undefined },
				createElement("button", { className: "mm_cardContent", type: "button", "aria-expanded": open, onClick: () => setOpen(!open) },
		...
		function McpSection() {
			...
			return createElement("div", { className: "mm_section" },
				...
				createElement("div", { className: "mm_cards" },
					filteredServers.map((s) => react.createElement(ServerRow, { server: s, onChanged: refresh, onEdit: () => { setEditingId(s.id); setView("edit"); }, key: s.id })),
				),
			);
		}
