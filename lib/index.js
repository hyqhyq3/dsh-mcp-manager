...
export function apply(ctx) {
  ...
  // Caps concurrent outbound HTTP requests (OAuth/token/notification calls) so a
  // burst of servers cannot exhaust local sockets/file descriptors.
  const MAX_CONCURRENT_HTTP_REQUESTS = 10;
  let activeHttpRequests = 0;
  const httpRequestQueue = [];
  function acquireHttpSlot() {
    if (activeHttpRequests < MAX_CONCURRENT_HTTP_REQUESTS) {
      activeHttpRequests++;
      return Promise.resolve();
    }
    return new Promise((resolve) => httpRequestQueue.push(resolve));
  }
  function releaseHttpSlot() {
    const next = httpRequestQueue.shift();
    if (next) next();
    else activeHttpRequests--;
  }
