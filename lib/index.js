        ...
        if (idMatch) {
          ...
          if (req.method === 'PUT' && !action) {
            ...
            if (type === 'stdio') {
              ...
              const serverUrl = String(body.url ?? '').trim();
              if (!/^https?:\/\//.test(serverUrl)) return json(res, 400, { error: 'url must be an http(s) URL' });
              next = { url: serverUrl, authMode: body.authMode === 'static' ? 'static' : 'oauth' };
              if (next.authMode === 'static') {
                // 留空表示保留原有 token(编辑表单不回填 token 出于安全)
                next.staticToken = String(body.token ?? '') || server.staticToken || '';
              }
            }
            ...
            if (type === 'stdio') {
              ...
              delete server.url; delete server.authMode; delete server.staticToken; delete server.oauth;
            } else {
              server.url = next.url;
              server.authMode = next.authMode;
              if (next.authMode === 'static') server.staticToken = next.staticToken; else delete server.staticToken;
            ...
            else if (server.authMode === 'static' && server.staticToken) await connect(server);
            else setLive(server.id, { status: 'needs-auth', error: server.authMode === 'static' ? 'missing token' : '' });
            return json(res, 200, { server: serverView(server) });
          }
